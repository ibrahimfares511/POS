...
class SalesController extends Controller
{
  ...
  public function view_sales(Request $request)
  {
    ...
    $order = Orders::max('id');
    $order ++ ;
    $listorder = Orders::where(['date'=>$date,'del'=>'0'])->orderBy('id', 'desc')->select('id')->get();
    $salsetype = Setting::get()->first();
    return view('sales.check',compact('customers','order','listorder','salsetype'));
  }
  ...
  public function Search_Name(Request $request){
    ...
    switch($request->op){
      case 'name':{
        $data = Products::with('Expired')->where('name', 'LIKE', '%' . $request['query'] . "%")
          ->get();
        return response()->json($data);
        }break;
      case 'code':{
        $data = Products::limit(1)->with('Expired')
  ...
  public function item_order(Request $request){
    ...
    function additem ($order,$date,$time,$user,$price,$quan,$disc,$pricelist,$customer,$opration,$op_ar,$item,$itemname,$no_item,$unit){
      if(Orders::limit(1)->where(['id'=>$order])->count() > 0){
        $orders = Orders::limit(1)->where(['id'=>$order])
          ->select(['subtotal','discount','profits','total'])
          ->first();
  
          $neworder = Orders::limit(1)->where(['id'=>$order])
            ->update([
              'date'     =>$date,
              'time'     =>$time,
              'user'     =>$user,
    ...
    switch($request->opration){
      case 'cash':{
        $product = Products::limit(1)->where('code',$request->item)
        ...
        if($product->total_pieces >= $total_pieces)
        {
          ...
          $pro_update = Products::limit(1)->where('code',$request->item)
            ->update([
  ...
  public function order_discount(Request $request){
    ...
    $total        = 0;
    $profits      = 0;
    $order = Orders::limit(1)->where('id',$request->order)
      ->select(['subtotal'])
      ->first();
    $details = DetailsOrder::where('order_id',$request->order)
      ->select('profits')
      ->get();
    ...
    switch($request->distype){
      case 'Value':{
        ...
        $pro_update = Orders::limit(1)->where('id',$request->order)
          ->update([
  ...
  public function search_order(Request $request){
    $orders = Orders::limit(1)->with('Details')->where(['id'=>$request->order, 'del'=>'0'])->get();
  ...
  public function save_balance(Request $request){
    if($request->customer > 0)
    {
      $update = Orders::limit(1)->where('id',$request->order)->update([
  ...
  public function del_element(Request $request){
    ...
    function delitem($serial,$element)
    {
      ...
      $order = Orders::limit(1)->where('id',$serial)->first();
      $details = DetailsOrder::limit(1)
        ->where(['order_id'=>$serial ,'no_item'=>$element])
        ->first();
      ...
      if($order->dis_type != Null)
      {
        ...
        switch($order->dis_type){
          case 'Value':{
            $pro_update = Orders::limit(1)->where('id',$serial)
              ->update([
                'discount'     => $ratio * 100,
                'dis_type'     => "Ratio",
        ...
        $pro_update = Orders::limit(1)->where('id',$order)
        ->update([
          'total'        => $newtotal,
          'profits'      => $newprofits,
          'subtotal'     => $newsubtotal,
      ...
      $details = DetailsOrder::limit(1)
        ...
        ->delete();
      // Check in last Order 
      if(DetailsOrder::limit(1)->where(['order_id'=>$serial])->count() == 0)
      {
        $del = Orders::limit(1)->where('id',$order)->update([
          'del'      => 1,
          'user_del' => $user
        ]);
      }
  ...
  {
    $update_product = '';
    $orderupdate = Orders::limit(1)->where('id',$request->order)
      ->update([
        'op'    => 'cash',
        'op_ar' =>'نقدي'
      ]);
    if($orderupdate){
      if(DetailsOrder::limit(1)->where(['order_id'=>$request->order])->count() > 0)
      {
        $update = DetailsOrder::where(['order_id'=>$request->order])->get();
        if($update){
          foreach($update as $item)
          {
            $pro_update = Products::limit(1)->where('code',$item->item)->first();
            ...
            if($item->unit == "قطعة"){
              ...
              $update_product = Products::limit(1)->where('code',$item->item)
              ->update([
  ...
  public function transfare_to_aggel(Request $request)
  {
    if($request->customer > '0'){
      ...
      if($update_product){
        $order = Orders::limit(1)->where('id',$request->order)->first();
        if($order->customer > 0)
        {
